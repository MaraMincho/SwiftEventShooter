...
struct SDKNetworkProvider<TargetType: NetworkTargetType>: Sendable {
    ...
    @discardableResult
    func request(_ type: TargetType, completion: @escaping (Result<Data, SDKNetworkError>) -> Void) -> URLSessionDataTaskInterface {
        do {
    ...
    private func requestResume(request: URLRequest, completion: @escaping (Result<Data, SDKNetworkError>) -> Void) -> URLSessionDataTaskInterface {
